...
export function GradeForm({ grade, open, onOpenChange }: { grade?: GradeRecord; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const { register, handleSubmit, reset } = useForm<GradePayload>({ defaultValues: emptyGrade })
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="left-auto right-3 w-[min(520px,calc(100vw-24px))] overflow-y-auto p-6">
        ...
        <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={(event) => void handleSubmit(submit)(event)}>
          <Select label="Student" registration={register('studentId', { required: true })} options={(students.data?.items ?? []).map((item) => ({ value: item.databaseId, label: `${item.name} (${item.id})` }))} />
          <Select label="Course" registration={register('courseId', { required: true })} options={(courses.data?.items ?? []).map((item) => ({ value: item.databaseId, label: item.title }))} />
          ...
          {(students.isError || courses.isError) && <p className="sm:col-span-2 text-sm font-semibold text-rose-600">Unable to load student or course choices.</p>}
          ...
          <div className="flex gap-3 sm:col-span-2"><Button disabled={mutation.isPending || students.isLoading || courses.isLoading} type="submit">{mutation.isPending ? 'Saving...' : grade ? 'Save Changes' : 'Add Grade'}</Button><Button type="button" variant="secondary" onClick={() => onOpenChange(false)}>Cancel</Button></div>
        </form>
      </SheetContent>
    </Sheet>
  )
